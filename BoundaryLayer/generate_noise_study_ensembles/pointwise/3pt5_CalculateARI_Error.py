...
def main():
    ...
    for i,noise in enumerate(noises):
        ...
        for j in range(no_runs): # choose the first no_runs trials to run sPCA
            ...
            ari = adjusted_rand_score(cluster_map.flatten(), cluster_map_ptwsBase.flatten())

            ari_err_arr.append(ari)

            

        # pct_err = np.median(ari_err_arr)

        print(f'For Noise {noise} and optimal alpha {optimal_alpha}, median ARI for Pointwise DB is {np.median(ari_err_arr)}, mean ARI is {np.mean(ari_err_arr)}')

        np.save(f'Error_Data/ARI_Error{noise}_Noise_{optimal_alpha}_alpha_pct_arr.npy', ari_err_arr)
