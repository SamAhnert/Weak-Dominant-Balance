...
MUST BE RUN LAST!!
(or use error files saved if not interested in fully rerunning ensemble)
...
def main():
    # noises = np.array([0.01, 1.0, 0.1,0.02, 0.025, 0.03, 0.04, 0.05,  0.25, 0.5]) # 
    ...
    # optimal_alpha_arr = np.array([187.5, 187.5, 5.0,312.5, 1.2, 1.0, 2.0, 7.5,  175.0, 250.0])
    ...
    spca_model_ptwsaBase = np.load(f'plots/2ndOrderFD_Derivatives_On_DNS_Grid/alpha_testing/pointwiseBaseline/spca_model.npy')
    ...
    for ii in range(sPCA_identified_relevant_terms_No_Noise_baseline.shape[0]):
        for jj in range(sPCA_identified_relevant_terms_No_Noise_baseline.shape[1]):
            ...
            sPCA_identified_relevant_terms_No_Noise_baseline[ii,jj,:] += spca_model_ptwsaBase[map_idx]
    ...
    for i,noise in enumerate(noises):
        ...
        pct_err = np.median(pct_err_arr)

        print(f'For Noise {noise} and optimal alpha {optimal_alpha}, best case error for Pointwise DB is {100 * pct_err}%, max_pct_err: {np.max(pct_err_arr)}')
        # print(f'min_pct_err: {np.min(pct_err_arr)}, , Q1: {np.percentile(pct_err_arr, 25)}, Q2: {np.percentile(pct_err_arr, 50)}, Q3: {np.percentile(pct_err_arr, 75)}')

        # aves pct_err array!
        # np.save(f'Error_Data/{noise}_Noise_{optimal_alpha}_alpha_pct_arr.npy', pct_err_arr)
        ...
        # Set max color to max potential error
        plt.pcolormesh(x,y,avg_error_distribution, vmin=0.0, vmax=0.6454779315350752, cmap='Reds')
        # plt.colorbar()
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        plt.savefig(f'Pointwise_Error_with_{noise}_noise.png', bbox_inches='tight', pad_inches=0.01,dpi=720)
        plt.clf()
