'''
Process ensemble of noisy Weak DB
'''
...
def main():
    ...
    for j in range(0,no_runs_Weak):
        ...
        for ii in range(sPCA_identified_relevant_terms.shape[0]):
            for jj in range(sPCA_identified_relevant_terms.shape[1]):
                ...
                # plot the identified terms of the first one
        if j == 0:
            print('cluster_idx range')
            RelevantTermPlots_dir = f'plots/ExpandedConvectionTerm/sPCA_models/Noise_{noise}/RelevantTerms/'
            # os.mkdir(RelevantTermPlots_dir)
            
            plt.figure(figsize = (15,4))
            plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc, sPCA_identified_relevant_terms[:,:,0])
            plt.title('UU_x is relevant here:')
            plt.colorbar()
            plt.savefig(RelevantTermPlots_dir + 'Streamwise_Advection_Test')
            plt.clf()
            plt.close()

            plt.figure(figsize = (15,4))
            plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc, sPCA_identified_relevant_terms[:,:,1])
            plt.title('VU_y is relevant here:')
            plt.colorbar()
            plt.savefig(RelevantTermPlots_dir + 'Spanwise_Advection_Test')
            plt.clf()
            plt.close()

            plt.figure(figsize = (15,4))
            plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc, sPCA_identified_relevant_terms[:,:,2])
            plt.title('LapU is relevant here:')
            plt.colorbar()
            plt.savefig(RelevantTermPlots_dir + 'Viscous_Term_Test')
            plt.clf()
            plt.close()

            plt.figure(figsize = (15,4))
            plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc, sPCA_identified_relevant_terms[:,:,3])
            plt.title('Pressure grad is relevant here:')
            plt.colorbar()
            plt.savefig(RelevantTermPlots_dir + 'Pressure_Grad_Test')
            plt.clf()
            plt.close()

            plt.figure(figsize = (15,4))
            plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc, sPCA_identified_relevant_terms[:,:,5])
            plt.title('RSS is relevant here:')
            plt.colorbar()
            plt.savefig(RelevantTermPlots_dir + 'R_Shear_Stress_Test')
            plt.clf()
            plt.close()

            plt.figure(figsize = (15,4))
            plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc, sPCA_identified_relevant_terms[:,:,4])
            plt.title('RNS is relevant here:')
            plt.colorbar()
            plt.savefig(RelevantTermPlots_dir + 'R_Normal_Stress_Test')
            plt.clf()
            plt.close()
    ...
    for i,noise in enumerate(noises):
        ...
        # Start by getting the appropriate grid for a given run of weak DB which will vary with the support used
        ...
        for j in range(0,no_runs_Weak):
            ...
            this_sPCA_identified_relevant_terms = np.zeros_like(sPCA_identified_relevant_terms)
            ...
            if take_weighted_sum:
                Y_diff = np.diff(Y_JH_DNS_trunc, axis=0)
                Y_diff = np.vstack((Y_diff[0,:],Y_diff))
                dx = X_JH_DNS_OG[1]-X_JH_DNS_OG[0]
                area = Y_diff * dx
                ...
                this_pct_err = np.sum(np.abs(this_sPCA_identified_relevant_terms - trunc_No_Noise_baseline)/this_sPCA_identified_relevant_terms.size)

                pct_err_arr.append(this_pct_err)
                
        pct_err_arr = np.array(pct_err_arr)

        np.save(f'plots/WeakDB/Error_Data/Noise_{noise}_support_{support_bound_x}_{support_bound_y}_alpha_{alpha}_noRunsWeak_{no_runs_Weak}_L0OfNoNoiseBaseline.npy',pct_err_arr)

        sPCA_identified_relevant_terms = sPCA_identified_relevant_terms / no_runs_Weak
        pct_err = np.sum(np.abs(sPCA_identified_relevant_terms - trunc_No_Noise_baseline))/sPCA_identified_relevant_terms.size

        print()
        print(f'avg pct error for weak DB with zero noise weak baseline for a noise level of {noise} and support of {support}: {np.mean(pct_err_arr)}')
        print(f'min_pct_err: {np.min(pct_err_arr)}, max_pct_err: {np.max(pct_err_arr)}, Q1: {np.percentile(pct_err_arr, 25)}, Q2: {np.percentile(pct_err_arr, 50)}, Q3: {np.percentile(pct_err_arr, 75)}')
        print()

        # raise Exception("weak err calculation success?")
