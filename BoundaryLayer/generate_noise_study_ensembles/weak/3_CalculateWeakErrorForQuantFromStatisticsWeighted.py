...
def main():
    ...
    load_dir = f'/home/sahnert/data/Dominant_Balance_Fine/plots/WeakDB/sPCA_models/Noise_{noise}/support_{support_bound_x}_{support_bound_y}/alph{alpha}/TF_degree_5/'
    ...
    for i,noise in enumerate(noises):
        ...
        for j in range(0,no_runs_Weak):
            ...
            if take_weighted_sum:
                ...
                error_distibution_arr.append(np.mean(np.abs(this_sPCA_identified_relevant_terms - trunc_No_Noise_baseline),axis=2))
        ...
        np.save(f'Error_Data/Noise_{noise}_support_{support_bound_x}_{support_bound_y}_alpha_{alpha}_noRunsWeak_{no_runs_Weak}_API_Error.npy',pct_err_arr)
        ...
        '''Plot Spatial Dist. Of Error'''
        error_distibution_arr = np.array(error_distibution_arr)

        avg_error_distribution = np.mean(error_distibution_arr, axis=0)

        # Save balance_model and balance maps & plot them too
        plt.figure(figsize = (11,4))
        plt.pcolormesh(X_JH_DNS_trunc,Y_JH_DNS_trunc,100*avg_error_distribution, vmin=0.0, vmax=100*0.6454779315350752, cmap='Reds')
        # plt.colorbar(format='%.0f%%')
        plt.gca().set_xticks([])
        plt.gca().set_yticks([])
        plt.savefig(f'Weak_Error_with_{noise}_noise.png', bbox_inches='tight', pad_inches=0.01,dpi=720)
        plt.clf()
