'''
Process ensemble of noisy Vanilla DB, data stored in ../plots/ExpandedConvectionTerm/ProofOfConceptFigure_VanVsWeakBLComparison/Vanilla_GMM_models/Derivatives_On_DNS_Grid/
'''
...
def main():
    ...
    for i,noise in enumerate(noises):
        ...
        for j in range(0,no_runs_Weak):
            ...
            nmi = normalized_mutual_info_score(cluster_idx.flatten(), trunc_final_cluster_idx_NoNoise.flatten())

            nmi_err_arr.append(nmi)
                
        nmi_err_arr = np.array(nmi_err_arr)

        np.save(f'Error_Data/Noise_{noise}_support_{support_bound_x}_{support_bound_y}_alpha_{alpha}_noRunsWeak_{no_runs_Weak}_NMI_Error.npy',nmi_err_arr)

        print()
        print(f'avg NMI for weak DB with zero noise weak baseline for a noise level of {noise} and support of {support}: {np.mean(nmi_err_arr)}')
