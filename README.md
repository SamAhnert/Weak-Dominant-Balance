...
The ```Duct``` folder is currently divided into ```pointwise_baseline``` and ```weak```. The duct data is stored in the general ```data``` folder in the repo to avoid redundant storage of the same data in multiple locations. 
...
```weak``` has the same folder structure, being split by the RANS and Vorticity Transport cases. The included code follows a slightly more involved workflow, where we include the code for our hyperparameter search in the first level of ```RANS``` and ```VortTrans``` for transparency. We have included the final clustering labels to recreate our results without re-running the full code from scratch (see ```plot_duct_RANS_errBased_sPCA.ipynb``` for RANS results and ```plot_results_errBased_sPCA_Weak_VortTransDuct.ipynb``` for VortTrans).

## Turbulent Wavy Channel Flow

The ```WavyChannel``` folder is again divided into ```pointwise_baseline``` and ```weak``` and PIV data is again stored in the general ```data``` folder to avoid redundant storage. 

```pointwise_baseline``` contains PIV and DNS examples, both under the streamwise RANS equation. The notebooks directly in ```WavyChannel``` are used to reproduce our Quantitative comparison of the two methods in ```Weak_WavyWall_QuantitativeComparison.ipynb```.

### pointwise DNS
In ```plotting```, the code to generate our results can be found. The results post GMM step can be found in ```results``` and after the sPCA is run on the GMM cluster, the results are placed in ```post_sPCA_arrays```, and those results can be loaded into the ```plotting/DNS_Pointwise_WavyChannel_Baseline.ipynb.ipynb``` notebook to recreate our results. The generation of our resulting GMM cluster labels is outlined in ```Oct_22_VanDBWavyWallDNS.py```.

### pointwise PIV

Code to recreate our results can be found in ```plotting/PIV_Pointwise_WavyChannel_Baseline.ipynb```, which will load cluster labels saved in ```post_sPCA_arrays```. A practitioner may generate their own GMMs and sPCA reductions using the same workflow we used, outlined in ```GenDBTrialData/Gen_GMMs_Pointwise_Baseline_WavyChannelPIV.py```.

### weak DNS & PIV

Weak dominant balance implementation for both these examples follows a very similar delineation of steps as the pointwise case. Code in the ```plotting``` folder of either the ```DNS``` and ```PIV``` folders will recreate the results of the paper. The process used to generate the GMM cluster labals can be found in the ```GenData``` directory of either of the ```DNS``` and ```PIV``` folders for those interested.
